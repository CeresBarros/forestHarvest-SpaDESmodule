...
#    `functionName` is a function's name in your module (e.g., `forestHarvestEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/cbarros.NRN/Documents/SpaDESintro/modules/forestHarvest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("forestHarvest")
  path <- list(modulePath = "C:/Users/cbarros.NRN/Documents/SpaDESintro/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    forestHarvest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("forestHarvestEvent1", envir = .GlobalEnv)) {
    simOutput <- forestHarvestEvent1(mySim)
  } else {
  ...
  if (exists("forestHarvestEvent2", envir = .GlobalEnv)) {
    simOutput <- forestHarvestEvent2(mySim)
  } else {
